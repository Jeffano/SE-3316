    ...
    function updateLists(collectionArray) {
        collectionArray.forEach(selectElement => {
            fetch('api/superheroes/collections/names')
                ...
                .then(collections => {
                    // Filter out specific collection names
                    const excludedCollections = ["SuperheroCollection", "SuperheroPowersCollection", "UserData"];
                    const filteredCollections = collections.filter(collectionName => !excludedCollections.includes(collectionName));
